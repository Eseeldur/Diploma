			...
			if($_POST){
				...
				if ($request==NULL) {
					...
					while ($row=$result->fetch()) {
						...
						$id_mass_name[$i]  = $row['Work_id'];
						$id_mass_avtor[$i] = $row['Work_id'];
						$id_mass_kluch[$i] = $row['Work_id'];
						$id_mass_gorod[$i] = $row['Work_id'];
						$id_mass_izdat[$i] = $row['Work_id'];
						$i++;
					}
					
					$id_count = count($id_mass_name); 
					...
					//проверка по циклу вхожения слов отдельно, но только по полю названия
					
					for ($i=0; $i<=$count; $i++) {
						for ($j=0; $j<=$id_count; $j++) {
							$sql = "SELECT Work_id, Name_of_work FROM works WHERE Work_id=".$id_mass_name[$j]." AND Name_of_work LIKE '%".$array[$i]."%'";
							$result = $db->query($sql);
							$result->setFetchMode(PDO::FETCH_ASSOC);
							$result->execute();
								//$id_mass2 = array();
					...
					//проверка по циклу вхожения слов отдельно, но теперь по полю автора
					
					for ($i=0; $i<=$count; $i++) {
						for ($j=0; $j<=$id_count; $j++) {
							$sql = "SELECT Id_author, Author's_name FROM news WHERE Work_id=".$id_mass[$j]." AND Name_of_work LIKE '%".$array[$i]."%'";
							$result = $db->query($sql);
							$result->setFetchMode(PDO::FETCH_ASSOC);
							$result->execute();
							$id_mass2 = array();
